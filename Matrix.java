...
>>>>>>> Stashed changes
    ...
    public double bicubicSplineInterpolation(double a,double b)
    {
...
=======
        ...
        //membuat matrix inverse dengan [M|I]
>>>>>>> Stashed changes
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                inverse.matrix[i][j] = this.matrix[i][j];
...
=======
        inverse.eselonBarisReduksi(); // gaus jordan

        // kembaliin nilai matrix
>>>>>>> Stashed changes
        for (int i = 0; i < n; i++) {
            zeroCount=0;
            for (int j = 0; j < n; j++) {
